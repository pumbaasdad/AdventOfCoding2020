...
namespace Day02
{
    ...
    {
        public static int PasswordPhilosophy1(string[] policies)
        {
            if (policies is null)
            {
            ...
            return policies.Count(Validate);
        }

        private static bool Validate(string policy)
        {
            var hyphen = policy.IndexOf('-');
            var space = policy.IndexOf(' ', hyphen);
            var colon = policy.IndexOf(':', space);

            var span = policy.AsSpan();

            var min = int.Parse(span.Slice(0, hyphen));
            var max = int.Parse(span.Slice(hyphen + 1, space - hyphen));
            var match = span[colon - 1];

            var count = 0;
            foreach (var c in span.Slice(colon + 1))
            {
                if (c == match)
                {
                    ++count;
        ...
        public static int PasswordPhilosophy2(string[] policies)
        {
            if (policies is null)
            {
                throw new ArgumentNullException(nameof(policies));
            ...
            return policies.Count(Validate2);
        }

        private static bool Validate2(string policy)
        {
            var hyphen = policy.IndexOf('-');
            var space = policy.IndexOf(' ', hyphen);
            var colon = policy.IndexOf(':', space);
            ...
            var first = int.Parse(span.Slice(0, hyphen));
            var second = int.Parse(span.Slice(hyphen + 1, space - hyphen));
            var match = span[colon - 1];

            span = span.Slice(colon + 1);

            return (span[first] == match) ^ (span[second] == match);
        }
        ...
        private static void Main()
        {
            ...
            Console.WriteLine(PasswordPhilosophy1(policies));
            Console.WriteLine(PasswordPhilosophy2(policies));
        }
    }
}